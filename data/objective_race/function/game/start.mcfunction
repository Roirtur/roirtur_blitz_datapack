...
execute unless score #state orace_state matches 0 run tellraw @s [{"text":"Objective Race is already busy. Use /function objective_race:game/reset if the round is stuck.","color":"red"}]
...
# Mark active players.
execute as @a[gamemode=!spectator] run function objective_race:players/mark_active

# Apply keep-inventory round setting.
execute if score #keepinv_setting orace_keep matches 1 run function objective_race:game/apply_keepinv_on
execute if score #keepinv_setting orace_keep matches 0 run function objective_race:game/apply_keepinv_off
...
function objective_race:game/pick_arena_center
...
function objective_race:game/freeze_players

# UI setup.
scoreboard objectives setdisplay sidebar orace_score
bossbar set objective_race:round max 5
bossbar set objective_race:round players @a[tag=orace.active]
bossbar set objective_race:round visible true
